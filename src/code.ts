...
export class StickyCode implements IDisposable {
  ...
  static createFromExistingCell(
    ...
    const tab = document.getElementsByName(cd.stickyContent.wrapperNode.id);
    tab[0].textContent = 'cd' + StickyCode.numCd;
    tab[0].innerHTML +=
    ...
    tab[0].setAttribute('name', 'cd' + StickyCode.numCd);

    cd.stickyContent.wrapperNode.id = 'cd' + StickyCode.numCd;
    StickyCode.numCd++;
